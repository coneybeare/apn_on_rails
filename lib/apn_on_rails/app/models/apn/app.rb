class APN::App < APN::Base
  ...
  def self.process_devices_for_cert(the_cert)
    ...
    APN::Feedback.devices(the_cert).each do |device|
      if device.updated_at < device.feedback_at
        puts "device #{device.id} -> #{device.updated_at} < #{device.feedback_at}"
        device.destroy
      else
        puts "device #{device.id} -> #{device.updated_at} not < #{device.feedback_at}"
      end
    end 
  end
